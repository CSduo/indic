const { db } = require("../lib/db/src/index");
const { submissionsTable, articlesTable, papersTable } = require("../lib/db/src/schema/index");
...
async function checkSubmissions() {
  try {
    ...
    for (const art of allArticles) {
      if (art.submissionId || art.authorName?.includes("chaitanya")) {
    ...
    for (const pap of allPapers) {
      if (pap.submissionId || pap.authorName?.includes("chaitanya")) {
